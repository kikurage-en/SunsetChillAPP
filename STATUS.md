...
4. **式の天井の再校正（2026-07-17 分析、提案中）** — 現行式が 85 点以上を出した 8 日の実測は中央値 68 で、80 を超えたのは 1 日（6/13 の 92）のみ。式は S 帯（85+）を 23% の日に出すが、実測の S 帯は 3%。天井の過大配点が式の楽観 bias（+4.9）の主因。
   - 候補C「**天井80、ただし超快晴（西 total<15% かつ low<5%）のみ90まで**」: N=35 で MAE 20.1→16.4、bias +4.9→+1.2、**良日（実測70+）の悪化ゼロ**（6/13 は例外条件で 90 を維持）。8日の系統的ファミリーに基づく校正であり N=1 フィットではない。
   - 効果が出る場所: 13:00 見出し（Vision無しで式がそのまま出る）、Vision欠測時のfallback、ブレンドの式2割。7/14 の「13:00 に 100(S) を予告 → 実測 65」型の誤報を防ぐ。
   - その他の項目は見直し不要と判断（降水ペナルティの保守性は意図した仕様、7/07型の中規模雲ギャップは N=1 のため保留、風・視程は誤差の証拠なし）。テーブル全体の再フィットはシーズン末（N≥50）に回帰的に行う方が過学習を避けられる。
   - → ユーザー判断待ち。
