...
`--`、天気参考欄です。残照の総合点と `vision_afterglow_score` は同じ値なので、
`残照：N / 100` の重複行は表示しません。
コメントは人手での現地確認を依頼せず、1行目にLINE表示用の最終Sunset期待度と
夕焼け要因だけを表示します。低層雲、高層雲、降水信号の矛盾、不確定理由の補足も
1行目にまとめます。2行目は蒸し暑さや強風など海辺の過ごしやすさに特記事項がある場合
だけ表示します。過ごしやすさは体感温度だけで決めつけず、
湿度、6:00〜日没の最高気温から夕方への気温低下、風、雨を組み合わせます。
湿度が高くても日中より気温が下がる場合や海風がある場合は、その救いも同じ行で伝えます。
日没後は現在時刻に近い気温を優先し、25℃台まで下がった場合は平均体感温度が高めでも
「かなり涼しい」と評価して、湿気や風などを補足します。
通常コメントとライブカメラ由来の所見は説明する文末に「っピ」を付け、高評価では明るく、
低評価では静かな調子に変化させます。ただし「わあっ！」「あああっ！」など独立した
感嘆詞には「っピ」を付けず、「わぁっ」は「わあっ」に正規化します。スコア総評、低層雲、
蒸し暑さ、強風、
高層雲、降水信号の矛盾、不確定理由には複数の台詞を持たせ、日付・実行時刻・
台詞カテゴリから決定的に選びます。
同じ観測の再実行では同じ文面になるためログの再現性を維持しつつ、13:00、17:00、
翌13:00と続く定期実行では同じ文面を連続させません。強い蒸し暑さは6種類、
軽い暑さの各条件は7種類です。特に繰り返しやすい「軽い蒸し暑さ＋気温低下・海風」は、
別々の定型文を連結せず、複数要素をまとめた短文7種類から選びます。25℃台で湿気と風が
ある場合も7種類です。不確定理由の補足は理由ごとに5種類です。気象条件とカメラの対比、カメラ観察、
総合結論に加え、日没後の事前見込み3段階×実際の夕焼け3段階の9通りにも各7種類の
結果表現を持たせ、同じ組み合わせが7日続いても同じ統合コメントを繰り返しません。
その他の定型文は各3種類以上です。
ライブカメラの文章は独立表示せず、夕焼けコメントへ統合します。日没前は気象条件と
カメラを2つの予測材料として対比し、表示用の総合判断へ着地させます。日没後は、同日に
...
「残照」を出さず、送信済み予測がない場合は画像結果だけを直接表現します。日没後の
Sunset期待度見出しは、取得できた場合に送信済み予測値を表示します。日没後のカメラ見出しは
「ライブカメラ夕焼け評価」とし、Visionの画像所見にある「残照」も自然な「夕焼け色」や
「光」へ置き換えて統合コメントに使います。元の画像所見と内部フェーズ名はログに保持します。
日没時・日没後のライブカメラ評価が70点以上（A相当以上）の場合は、夕焼け結果と
過ごしやすさの後に空行を1つ挟み、励まし・休息・気分転換の一言を追加します。
汎用文24種類を日付で決定的に切り替え、24日間は同じ文を繰り返しません。過ごしやすさの
特記事項がない場合は、暑さ・海風・涼しさ・日中からの気温低下に合う12種類の天候連動文も
候補にします。過ごしやすさ行がある場合は同じ天候要素を励まし文で繰り返しません。
13:00 / 17:00 の予測では、日没前後で雨予報が大きく変わる場合、
にわか雨・雷雨、予報値の欠測や予報間の大幅な差、式とライブカメラAI予測の25点以上の
乖離などを検知しても、スコアに基づく夕焼けの見込みは残します。その後に、
「天気が変わりやすい」「不安定な空模様」などの弱気な補足を添えます。
...
降水確率80%以上の通常ペナルティは-60です。ただし、評価時間帯の予想雨量が0、代表天気コードが0/1、西側総雲量が50%未満、西側低層雲が30%未満をすべて満たす場合だけ、矛盾信号として暫定的に-25へ緩和します。「雨なし」と断定する条件ではなく、LINEでは夕焼けの見込みを示したうえで、天気が変わりやすい旨を補足します。湿度は霞の代理としてSunset期待度へ直接減点せず、Chill指数と保存済みの視程で扱います。
