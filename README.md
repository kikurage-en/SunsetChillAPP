...
ある場合も7種類です。不確定理由の補足は理由ごとに5種類です。日没前の気象条件と
カメラ評価は、一致・カメラが慎重・カメラが楽観の3方向×最終評価3段階の9通りに
各7種類の短文を持たせます。日没後の事前見込み3段階×実際の夕焼け3段階の9通りにも
各7種類の結果表現を持たせ、同じ組み合わせが7日続いても同じコメントを繰り返しません。
その他の定型文は各3種類以上です。
