...
不確定理由の補足は理由ごとに5種類です。気象条件とカメラの対比、カメラ観察、
総合結論、期待を下回る・期待どおり・期待を上回る結果表現は各7種類とし、同じ評価帯が
7日続いても同じ統合コメントを繰り返しません。その他の定型文は各3種類です。
ライブカメラの文章は独立表示せず、夕焼けコメントへ統合します。日没前は気象条件と
カメラを2つの予測材料として対比し、表示用の総合判断へ着地させます。日没後は、同日に
実際に送信された17:00（冬季など17:00が日没後なら13:00）の表示用期待度を読み出し、
「17時は〜だったけれど、実際の残照は〜」のように予測と画像結果を振り返ります。
取得できない場合は「気象条件の評価は〜」と表現し、過去予測とは扱いません。日没後の
Sunset期待度見出しも、取得できた場合はこの送信済み予測値を表示します。Visionが生成した
画像所見そのものはログへ残し、日没時・残照時の統合コメントで画像の事実として使います。
