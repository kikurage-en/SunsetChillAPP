...
保存スキーマは80列です。68列版からは、純式の加減点内訳、キャップ前のブレンド値と上方キャップ発動有無、画像の取得元・取得時刻・SHA-256を表す12列を末尾へ追加しています。59列版・68列版を含む既存のGoogle Sheetsは、ヘッダーが現行構成のprefixと一致すれば列幅を80列まで自動拡張して移行します。既存CSVは自動変更しないため、末尾へ新しい列を追加してください（不一致時は `ConfigError` で停止）。
...
降水確率80%以上の通常ペナルティは-60です。ただし、評価時間帯の予想雨量が0、代表天気コードが0/1、西側総雲量が50%未満、西側低層雲が30%未満をすべて満たす場合だけ、矛盾信号として暫定的に-25へ緩和します。「雨なし」と断定する条件ではなく、LINEには「空の変化が大きく、予測に幅があります」とだけ簡潔に表示します。湿度は霞の代理としてSunset期待度へ直接減点せず、Chill指数と保存済みの視程で扱います。
...
純式とVision予測の差が30点以上、または上方キャップが表示値へ影響した場合、LINEには内部の式・キャップ名を並べず、`※空の変化が大きく、予測に幅があります。` の1行だけを表示します。原因分析用の加減点、キャップ前の値、キャップ発動有無はログへ分離します。
