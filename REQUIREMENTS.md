...
- Vision解析がある場合、1行目へ気象条件とカメラ評価を統合する。日没前は両方を
  予測材料として「天気の条件では〜だったけれど、今の空は〜。総合すると〜」と
...
- Visionが生成した画像所見はログへ保持し、日没時・残照時は予測対結果の統合コメント内で
  ...
  気象条件との対比、カメラ観察、総合結論、事前の見込み3段階×実際の夕焼け3段階の
  結果表現は各7種類用意し、`date` と `run_time` から決定的に切り替える。同じ組み合わせが
  7日続いても同じ統合コメントを繰り返さない。
...
- 日没前の各評価帯と、日没後の事前見込み3段階×実際の夕焼け3段階の各組み合わせで、
  統合コメントが7日間重複しないことをテストする。低い見込みと低い結果の組み合わせでは
  「期待」を使わないこともテストする。
