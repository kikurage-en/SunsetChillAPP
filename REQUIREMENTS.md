...
- 降水確率80%以上でも、評価時間帯の予想雨量0、晴天コード0/1、西側総雲量50%未満、
  ...
  この条件は「雨なし」の断定ではなく、本文には予測に幅がある旨だけを簡潔に表示する。
...
純式とVision予測の差が30点以上、または上方キャップが表示値へ影響した場合は、
LINEへ `※空の変化が大きく、予測に幅があります。` と表示する。純式、キャップ前の値、
上方キャップ名などの内部情報はLINEへ表示せず、診断ログへ保存する。
...
- 日没前のコメントは予測表現とし、日没時・残照フェーズでは「見込み」などの予測表現を
  使わず、その時点の条件として表現する。
- 高い降水確率と雨量・晴天コード・西空の薄い雲が食い違う場合は、予測に幅がある旨を
  簡潔な表現でコメントへ含める。
- 純式とVision予測の差が30点以上、または上方キャップが表示値へ影響した場合も同じ
  簡潔な注記を表示し、内部の式・キャップ名は本文へ表示しない。
...
保存スキーマは次の80列とし、順序は `src/zushi_chill/storage.py` の `CSV_COLUMNS` を正とする。
...
- CSVの80列出力とGoogle Sheetsの旧ヘッダー・列幅移行をテストする。
...
- サンプル蓄積後の主予測信号選抜
- 日没時発色と残照を別々の目的変数として校正し、単一の正解値へ統合しない。
- 16時・17時など複数画像による雲の変化分析。追加ジョブ・API利用を伴うため、
  有効性を確認し運用承認を得てから有効化する。
