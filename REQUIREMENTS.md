...
- Vision解析がある場合、1行目へ気象条件とカメラ評価を統合する。日没前は両方を
  ...
  （17:00、13:00）で取得し、「17時は〜だったけれど、実際の残照は〜」のように
  予測と結果を比較する。送信済み予測を取得できない場合だけ「気象条件の評価は〜」へ
  フォールバックし、過去予測だったかのようには表現しない。日没後の本文見出しも、
  取得できた場合は同じ送信済み予測の表示用Sunset期待度を使う。
...
- Visionが生成した画像所見はログへ保持し、日没時・残照時は予測対結果の統合コメント内で
  ...
  逆方向の感嘆詞は統合時に除く。気象条件との対比、カメラ観察、総合結論、期待を
  下回る・期待どおり・期待を上回る結果表現は各7種類用意し、`date` と `run_time` から
  決定的に切り替える。同じ評価帯が7日続いても同じ統合コメントを繰り返さない。
...
- 日没前の各評価帯と日没後の下振れ・一致・上振れで、統合コメントが7日間重複しない
  ことをテストする。
- 日没後に送信済みの日没前表示用期待度を取得して見出しと振り返りへ使い、未送信・欠測時は
  現在の気象条件との比較へフォールバックすることをCSV・Google Sheets・本文でテストする。
